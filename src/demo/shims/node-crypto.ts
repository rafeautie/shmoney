// The two node:crypto calls the main-process code makes, in the browser. Web
// Crypto's digest is async, and import dedupe hashes synchronously, so
// SHA-256 is implemented here; ids then match what the desktop app computes.
...
export function createHash(algorithm: string): {
  ...
  return {
    update: (data) => ({ digest: () => sha256(new TextEncoder().encode(data)) })
  }
}
